function subplotDist(nrows, ncols, row, plotInfo, gstats, fieldname, names,...
                        batchIdx, batchLabel)
    ...
    for i = 1:nBatches
        for j = 1:numel(batchIdx{i})
            h(n) = plot(bins(n,:), dist(n,:), 'Color', cmap(n,:), 'LineWidth', 2);
            n = n+1;
        end
    end
    hold off;
    ...
    for i = 1:nBatches
        for j = 1:numel(batchIdx{i})
            h(n) = plot(bins(n,:), cumsum(dist(n,:)), 'Color', cmap(n,:), 'LineWidth', 2);
            n = n+1;
        end
    end
    hold off;
