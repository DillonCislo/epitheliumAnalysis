    ...
    function [cells,bonds] = BondsConversion(VorCells,NVertices)
        ...
        for i = 1 : NCells
            ...
            for j = 1 : (length(VorCells{i})-1)
                ...
                % S(v2,v1) ~= i exludes the case of the two sided cell,
                ...
                % inside bond, not two sided
                % if a two sided cell borders this cell, it is even
                % possible that S(v1, v2) ~= i but that doesn't matter
                if S(v2,v1) ~= 0 && S(v2,v1) ~= i

                    bonds(Counter,:) = [VorCells{i}(j) VorCells{i}(j+1) i S(v2,v1)];

                % outside bond
                elseif S(v2,v1) == 0 
                    
                    bonds(Counter,:) = [VorCells{i}(j) VorCells{i}(j+1) i 0];
	
                % two sided cell, must have one outside
                elseif S(v2,v1) == i
                    ...
                    % one tells us the neighboring cell
                        ...
                        bonds(Counter,:) = [VorCells{i}(j) VorCells{i}(j+1) i bonds(antiPreBond,3)];
                    end
                end
